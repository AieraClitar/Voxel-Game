...
export const Textures = {
    generate: function(type) {
        ...
        else if (type === 'grass_side') { fillBase('#6d4c41'); addNoise(30, 0.3); ctx.fillStyle = '#558b2f'; ctx.fillRect(0, 0, 16, 4); for(let x=0; x<16; x++) { let drop = Math.floor(Math.random() * 4); ctx.fillRect(x, 4, 1, drop); } }
        ...
        const texture = new THREE.CanvasTexture(canvas); texture.magFilter = isTool ? THREE.LinearFilter : THREE.NearestFilter; return texture;
    }
};