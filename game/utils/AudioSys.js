export const AudioSys = {
    ...
    // ✨ PHASE 1: REALISTIC WEAPON SOUNDS
    shootGun: function(dist = 0) { 
        const v = this.getDistVol(dist, 0.4);
        this.playTone(150, 'sine', 0.3, v, 100); // Heavy bass thump
        this.playNoise(0.2, v, 4000);            // Sharp gunpowder crack
    },
    shootBow: function(dist = 0) { 
        if(!this.ctx) return;
        const v = this.getDistVol(dist, 0.25);
        
        // Pitch-bending string twang
        const osc = this.ctx.createOscillator(); const gain = this.ctx.createGain();
        osc.type = 'triangle'; 
        osc.frequency.setValueAtTime(150, this.ctx.currentTime);
        osc.frequency.linearRampToValueAtTime(350, this.ctx.currentTime + 0.15); // Twang snaps upwards
        ...
        this.playNoise(0.15, v * 0.4, 1000); // Arrow woosh through the air
    },
    shootCrossbow: function(dist = 0) { 
        const v = this.getDistVol(dist, 0.3);
        this.playTone(800, 'square', 0.05, v * 0.8, 400); // Mechanical metal clack
        this.playTone(200, 'sawtooth', 0.2, v, 100);      // Heavy bolt thwack
