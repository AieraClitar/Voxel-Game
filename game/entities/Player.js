...
export class Player {
    constructor(camera, domElement, world) {
        ...
        this.cameraMode = 0; 
        this.tpsCameraBack = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
        this.tpsCameraBack.position.set(0, 0.5, 4); 
        this.camera.add(this.tpsCameraBack);

        this.tpsCameraFront = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
        this.tpsCameraFront.position.set(0, 0.5, -4); 
        this.tpsCameraFront.rotation.y = Math.PI; 
        this.camera.add(this.tpsCameraFront);
        ...
        this.joyMove = new THREE.Vector2(0, 0);
        ...
        iconCam.position.set(0, 0, 2.5); iconCam.lookAt(0, 0, 0);
        iconScene.add(new THREE.AmbientLight(0xffffff, 1.0));
        const dirLight = new THREE.DirectionalLight(0xffffff, 1.0);
        dirLight.position.set(5, 10, 5); iconScene.add(dirLight);
        ...
        for (const typeStr in this.world.itemMaterials) {
            const mat = this.world.itemMaterials[typeStr];
            let geo, mesh;
            ...
            this.uiIcons[typeStr] = img;
            iconScene.remove(mesh); geo.dispose();
        }
        ...
        this.arm = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.6, 0.2), matSkin);
        this.arm.position.set(0.35, -0.3, -0.4); 
        ...
        this.muzzleFlash.position.set(0, 0.3, -0.4);
        this.arm.add(this.muzzleFlash);
        ...
        this.head = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.5, 0.5), headMaterials);
        this.head.position.set(0, 0, 0); 
        this.torso = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.75, 0.25), new THREE.MeshLambertMaterial({color: 0x3333aa}));
        this.torso.position.set(0, -0.625, 0); 
        
        this.armL = new THREE.Group(); this.armL.position.set(-0.425, -0.25, 0); 
        const armLMesh = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.75, 0.25), matSkin); armLMesh.position.y = -0.375; this.armL.add(armLMesh);
        
        this.armR_3rd = new THREE.Group(); this.armR_3rd.position.set(0.425, -0.25, 0);
        const armRMesh = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.75, 0.25), matSkin); armRMesh.position.y = -0.375; this.armR_3rd.add(armRMesh);
        
        this.heldBlock_3rd = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.1,0.1), new THREE.MeshLambertMaterial({color: 0xffffff}));
        this.armR_3rd.add(this.heldBlock_3rd);

        this.legL = new THREE.Group(); this.legL.position.set(-0.15, -1.0, 0); 
        const legLMesh = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.5, 0.25), matPants); legLMesh.position.y = -0.25; this.legL.add(legLMesh);
        
        this.legR = new THREE.Group(); this.legR.position.set(0.15, -1.0, 0); 
        const legRMesh = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.5, 0.25), matPants); legRMesh.position.y = -0.25; this.legR.add(legRMesh);
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            if (this.isDistributing && this.draggedItem.type) {
                ...
                for (let el of els) {
                    if (el.classList.contains('inv-item') && el.dataset.type === 'crafting') { this.distributeItemToSlot(parseInt(el.dataset.index)); break; }
                }
            }
        });
        
        document.addEventListener('mousedown', (e) => { 
            if (e.target.id === 'close-inv') return;
            if (this.draggedItem.type) { this.isDistributing = true; this.distributedSlots.clear(); } 
        });
    ...
    getStackLimit(type) {
        ...
        if (type === 'torch') return 32;
        return 64; 
    }

    getActiveCamera() {
        if (this.cameraMode === 1) return this.tpsCameraBack;
        if (this.cameraMode === 2) return this.tpsCameraFront;
        return this.camera; 
    }
    ...
    dropSelectedItem() {
        ...
        if (item && item.count > 0) {
            ...
            this.world.spawnItemDrop(dropPos.x, dropPos.y, dropPos.z, item.type);
            item.count--;
            if (item.count <= 0) item.type = null;
            this.updateInventoryUI();
            if (AudioSys && AudioSys.playNoise) AudioSys.playNoise(0.1, 0.1, 400); 
        }
    }
    ...
    takeDamage(amount, source) {
        ...
        flash.style.backgroundColor = 'rgba(255, 0, 0, 0.4)'; flash.style.pointerEvents = 'none'; flash.style.zIndex = '9999'; flash.style.transition = 'opacity 0.2s';
        ...
        if (this.health <= 0) {
            ...
            this.camera.position.set(16, this.world.getSurfaceHeight(16,16)+2, 16);
    ...
    initControls() {
        document.addEventListener('keydown', (e) => {
            ...
            switch(e.code){
                case 'KeyW': this.moveForward = true; break; case 'KeyA': this.moveLeft = true; break;
                case 'KeyS': this.moveBackward = true; break; case 'KeyD': this.moveRight = true; break;
                ...
                case 'Digit3': this.setHotbarSlot(2); break; case 'Digit4': this.setHotbarSlot(3); break; case 'Digit5': this.setHotbarSlot(4); break;
            }
        });
        document.addEventListener('keyup', (e) => {
            switch(e.code){ case 'KeyW': this.moveForward = false; break; case 'KeyA': this.moveLeft = false; break; case 'KeyS': this.moveBackward = false; break; case 'KeyD': this.moveRight = false; break; }
        });
        ...
        document.addEventListener('wheel', (e) => {
            if (this.controls.isLocked) { 
                ...
                let n = this.selectedSlot + Math.sign(e.deltaY); 
                if(n>4)n=0; if(n<0)n=4; 
                this.setHotbarSlot(n); 
            }
        });

        const closeBtn = document.getElementById('close-inv');
        if (closeBtn) {
            closeBtn.onclick = (e) => { e.stopPropagation(); this.toggleInventory(); };
            closeBtn.ontouchstart = (e) => { e.preventDefault(); e.stopPropagation(); this.toggleInventory(); };
        }

        const joyZone = document.getElementById('m-joystick-zone');
        const joyKnob = document.getElementById('m-joystick-knob');
        let joyTouchId = null;
        ...
        bindTouch('m-jump', () => { if(this.canJump){ this.velocity.y = this.jumpForce; this.canJump = false; } });
        bindTouch('m-inv', () => this.toggleInventory());
        bindTouch('m-place', () => { if(this.gameActive) this.onInteract(2); });
        bindTouch('m-drop', () => { this.dropSelectedItem(); }); 

        const lookZone = document.getElementById('m-look-zone');
        let lookTouchId = null; let lastTouchX = 0; let lastTouchY = 0; let touchStartTime = 0; let isSwiping = false;
        ...
        if (lookZone) {
            lookZone.addEventListener('touchstart', (e) => {
                ...
                for (let touch of e.changedTouches) {
                    if (lookTouchId === null) {
                        lookTouchId = touch.identifier; lastTouchX = touch.pageX; lastTouchY = touch.pageY; touchStartTime = performance.now(); isSwiping = false;
                        this.miningTimeout = setTimeout(() => { if (!isSwiping && this.gameActive) this.onInteract(0); }, 250);
                    }
                }
            }, {passive: false});
                ...
                for (let touch of e.touches) {
                    if (touch.identifier === lookTouchId) {
                        ...
                        if (this.gameActive && !this.isInvOpen) {
                            ...
                            this._euler.y -= dx * 0.005; this._euler.x -= dy * 0.005;
            ...
            const endLook = (e) => {
                for (let touch of e.changedTouches) {
                    if (touch.identifier === lookTouchId) {
                        ...
                        if (!isSwiping && performance.now() - touchStartTime < 250 && this.gameActive) { this.onInteract(0); setTimeout(() => this.stopMining(), 100); }
                        this.stopMining(); lookTouchId = null;
                    }
                }
            };
        ...
        document.addEventListener('touchmove', (e) => {
            ...
            if (dragEl && dragEl.style.display === 'block' && e.touches.length > 0) {
                ...
                if (this.isDistributing && this.draggedItem.type) {
                    ...
                    if (el && el.classList.contains('inv-item') && el.dataset.type === 'crafting') { this.distributeItemToSlot(parseInt(el.dataset.index)); }
                }
            }
        }, {passive: false}); 
    }
    ...
    updateJoystick(touch, rectZone, knob) {
        const rect = rectZone.getBoundingClientRect(); const cx = rect.left + rect.width / 2; const cy = rect.top + rect.height / 2;
        let dx = touch.pageX - cx; let dy = touch.pageY - cy;
        ...
        this.joyMove.set(kx / maxDist, ky / maxDist);
    }

    bindSlotEvents(slotDiv, itemType, index, sourceArrayStr) {
        slotDiv.onmousedown = (e) => {
            e.preventDefault(); e.stopPropagation();
            if (!this.isInvOpen) { if (sourceArrayStr === 'hotbar') this.setHotbarSlot(index); return; }
            if (e.button === 0) this.handleSlotClick(sourceArrayStr, index); else this.handleSlotRightClick(sourceArrayStr, index);
        };
        ...
        slotDiv.onmouseenter = (e) => {
            if (itemType) {
                ...
                tt.innerText = itemType.replace(/_/g, ' ').toUpperCase(); tt.style.display = 'block';
                const rect = slotDiv.getBoundingClientRect(); tt.style.left = rect.left + 'px'; tt.style.top = (rect.top - 30) + 'px';
            }
            if (this.isDistributing && sourceArrayStr === 'crafting') { this.distributeItemToSlot(index); }
        };
        ...
        slotDiv.ontouchstart = (e) => {
            ...
            const dragEl = document.getElementById('dragged-item');
            if (dragEl && e.touches.length > 0) { startX = e.touches[0].pageX; startY = e.touches[0].pageY; dragEl.style.left = startX + 'px'; dragEl.style.top = startY + 'px'; }
            
            if (this.draggedItem.type) { 
                this.isDistributing = true; this.distributedSlots.clear(); 
                if (sourceArrayStr === 'crafting') this.distributeItemToSlot(index); 
            }

            touchTimer = setTimeout(() => { this.handleSlotRightClick(sourceArrayStr, index); touchTimer = null; }, 400); 
        };

        slotDiv.ontouchmove = (e) => { 
            if (touchTimer && e.touches.length > 0) { 
                if (Math.abs(e.touches[0].pageX - startX) > 10 || Math.abs(e.touches[0].pageY - startY) > 10) { clearTimeout(touchTimer); touchTimer = null; } 
            } 
        };
        
        slotDiv.ontouchend = (e) => { 
            e.preventDefault(); e.stopPropagation(); 
            this.isDistributing = false; 
            if (!this.isInvOpen) return; 
            if (touchTimer) { 
                clearTimeout(touchTimer); 
                this.handleSlotClick(sourceArrayStr, index); 
            } 
        };
    }
    ...
    distributeItemToSlot(index) {
        ...
        const limit = this.getStackLimit(this.draggedItem.type);
        
        if (!slot.type || slot.type === this.draggedItem.type) {
            if (slot.count < limit) {
                slot.type = this.draggedItem.type; slot.count++; this.draggedItem.count--;
                this.distributedSlots.add(index);
                if (this.draggedItem.count <= 0) this.draggedItem = { type: null, count: 0 };
                ...
                for(let x=0; x<=3-w; x++) {
                    ...
                    for(let py=0; py<h; py++) {
                        for(let px=0; px<w; px++) {
                            let gItem = g[(y+py)*3 + (x+px)];
                            let pItem = p[py*w + px];
                            if (pItem === 'ANY_PLANKS') {
                                if (gItem !== 'oak_planks' && gItem !== 'birch_planks') match = false;
                                pCount++;
                            } else if (pItem !== null) {
                                if (gItem !== pItem) match = false;
                                pCount++;
                            }
                        }
                    }
    ...
    handleSlotClick(slotType, index) {
        let targetArray = slotType === 'hotbar' ? this.inventory : (slotType === 'main' ? this.mainInventory : this.craftingGrid);
        let targetSlot = targetArray[index];
        
        if (this.draggedItem.type) {
            if (targetSlot.type === this.draggedItem.type) { 
                let limit = this.getStackLimit(targetSlot.type);
                let space = limit - targetSlot.count;
                if (space > 0) {
                    let transfer = Math.min(space, this.draggedItem.count);
                ...
                targetSlot.type = this.draggedItem.type; targetSlot.count = this.draggedItem.count; 
                this.draggedItem = temp; 
            }
        } else {
            if (targetSlot.type) { this.draggedItem = { type: targetSlot.type, count: targetSlot.count }; targetSlot.type = null; targetSlot.count = 0; }
        }
    ...
    handleCraftingResultClick() {
        if (!this.craftingResult.type) return;
        let limit = this.getStackLimit(this.craftingResult.type);
    ...
    toggleInventory() {
        ...
        if (this.gameActive && !this.isInvOpen) { 
            ...
            if (this.draggedItem.type) { 
                const dropDir = new THREE.Vector3(0, 0, -1).applyQuaternion(this.camera.quaternion);
                const dropPos = this.camera.position.clone().add(dropDir.multiplyScalar(1.5));
                for(let c=0; c < this.draggedItem.count; c++) this.world.spawnItemDrop(dropPos.x, dropPos.y, dropPos.z, this.draggedItem.type); 
                this.draggedItem = {type: null, count: 0}; 
            }
            for (let i = 0; i < 9; i++) { 
                if (this.craftingGrid[i].type) { 
                    const dropDir = new THREE.Vector3(0, 0, -1).applyQuaternion(this.camera.quaternion);
                    const dropPos = this.camera.position.clone().add(dropDir.multiplyScalar(1.5));
                    for(let c=0; c < this.craftingGrid[i].count; c++) this.world.spawnItemDrop(dropPos.x, dropPos.y, dropPos.z, this.craftingGrid[i].type); 
    ...
    pickupItem(type) {
        const limit = this.getStackLimit(type);
        if (AudioSys && AudioSys.playTone) AudioSys.playTone(600, 'sine', 0.1, 0.1); 
    ...
    updateInventoryUI() {
        const renderSlot = (slotDiv, item, index, sourceStr) => {
            ...
            targetSlot.dataset.index = index; targetSlot.dataset.type = sourceStr;
            this.bindSlotEvents(targetSlot, item.type, index, sourceStr);
        ...
        if (grid) { grid.innerHTML = ''; for(let i=0; i<16; i++) { const slot = document.createElement('div'); slot.className = 'inv-item'; renderSlot(slot, this.mainInventory[i], i, 'main'); grid.appendChild(slot); } }
        ...
        if (craftGrid) {
            ...
            for(let i=0; i<9; i++) {
                const slot = document.createElement('div'); slot.className = 'inv-item';
                if (!this.isCraftingTableOpen && [2,5,6,7,8].includes(i)) slot.classList.add('hidden-slot');
                renderSlot(slot, this.craftingGrid[i], i, 'crafting'); craftGrid.appendChild(slot);
            }
        }

        const resultSlot = document.getElementById('crafting-result');
        if (resultSlot) {
            let newResult = resultSlot;
            if (resultSlot.parentNode) { newResult = resultSlot.cloneNode(false); resultSlot.parentNode.replaceChild(newResult, resultSlot); }
            newResult.innerHTML = '';
            ...
            if (this.craftingResult.type) {
                if (this.uiIcons[this.craftingResult.type]) newResult.appendChild(this.uiIcons[this.craftingResult.type].cloneNode());
                const countSpan = document.createElement('span'); countSpan.className = 'item-count'; countSpan.innerText = this.craftingResult.count; newResult.appendChild(countSpan);
            }
        }
        ...
        if (dragEl) {
            ...
            if (this.draggedItem.type !== null && this.draggedItem.count > 0) {
                ...
                const countSpan = document.createElement('span'); countSpan.className = 'item-count'; countSpan.innerText = this.draggedItem.count; dragEl.appendChild(countSpan);
        ...
        if (selected && selected.type !== null && selected.count > 0) {
            this.heldBlock.visible = true; this.heldBlock_3rd.visible = true;
            let mat = this.world.itemMaterials[selected.type] || this.world.itemMaterials['stone'];
            if (this.heldBlock.geometry) this.heldBlock.geometry.dispose();
            if (this.heldBlock_3rd.geometry) this.heldBlock_3rd.geometry.dispose();

            if (isTool(selected.type)) {
                this.heldBlock.geometry = new THREE.PlaneGeometry(0.8, 0.8); this.heldBlock.geometry.translate(0.4, 0.4, 0); 
                this.heldBlock.material = mat; this.heldBlock.position.set(-0.05, 0.3, -0.1); this.heldBlock.rotation.set(0, -Math.PI / 2, 0); 
                this.heldBlock_3rd.geometry = new THREE.PlaneGeometry(1.0, 1.0); this.heldBlock_3rd.geometry.translate(0.5, 0.5, 0); 
                this.heldBlock_3rd.material = mat; this.heldBlock_3rd.position.set(0, -0.75, -0.15); this.heldBlock_3rd.rotation.set(-Math.PI / 8, -Math.PI / 2, 0); 
            } else if (selected.type === 'torch') {
                this.heldBlock.geometry = new THREE.BoxGeometry(0.05, 0.3, 0.05); this.heldBlock.geometry.translate(0, 0.15, 0); 
                this.heldBlock.material = mat; this.heldBlock.position.set(0, 0.3, -0.1); this.heldBlock.rotation.set(Math.PI / 8, 0, 0); 
                this.heldBlock_3rd.geometry = new THREE.BoxGeometry(0.08, 0.4, 0.08); this.heldBlock_3rd.geometry.translate(0, 0.2, 0); 
                this.heldBlock_3rd.material = mat; this.heldBlock_3rd.position.set(0, -0.75, -0.15); this.heldBlock_3rd.rotation.set(-Math.PI / 8, 0, 0); 
            } else {
                this.heldBlock.geometry = new THREE.BoxGeometry(0.15, 0.15, 0.15); this.heldBlock.geometry.translate(0, 0.075, 0); 
                this.heldBlock.material = mat; this.heldBlock.position.set(0, 0.3, -0.1); this.heldBlock.rotation.set(0, Math.PI / 4, 0); 
                this.heldBlock_3rd.geometry = new THREE.BoxGeometry(0.25, 0.25, 0.25); this.heldBlock_3rd.geometry.translate(0, 0.125, 0);
                this.heldBlock_3rd.material = mat; this.heldBlock_3rd.position.set(0, -0.75, -0.15); this.heldBlock_3rd.rotation.set(0, Math.PI / 4, 0); 
            }
    ...
    showHotbarName() {
        ...
        if (item && item.type) {
            const tt = document.getElementById('item-tooltip');
            tt.innerText = item.type.replace(/_/g, ' ').toUpperCase(); tt.style.display = 'block';
            const activeSlot = document.getElementById(`slot-${this.selectedSlot + 1}`);
            if(activeSlot) { const rect = activeSlot.getBoundingClientRect(); tt.style.left = rect.left + 'px'; tt.style.top = (rect.top - 30) + 'px'; }
    ...
    onInteract(buttonIdx) {
        ...
        for (let intersect of intersects) {
            ...
            if (intersect.object.isInstancedMesh) {
                ...
                if (!pos) continue; bx = pos.x; by = pos.y; bz = pos.z;
                ...
                if (buttonIdx === 0) { 
                    let hitMob = intersect.object.userData.mobRef;
                    let dmg = 5; 
            ...
            if (buttonIdx === 0) { 
                ...
                if (type === 'torch') { this.world.removeBlock(bx, by, bz); this.world.spawnItemDrop(bx, by, bz, 'torch'); if(AudioSys && AudioSys.breakBlock) AudioSys.breakBlock(); break; }
                ...
                const selected = this.inventory[this.selectedSlot];
                if (selected && selected.type !== null && selected.count > 0 && !selected.type.includes('sword') && !selected.type.includes('pickaxe') && !selected.type.includes('axe') && !selected.type.includes('shovel') && selected.type !== 'stick' && selected.type !== 'bow' && selected.type !== 'crossbow' && selected.type !== 'gun') {
                    if (selected.type === 'torch' && type === 'leaves') break;
                    if (type === 'torch') break; 
                    const normal = intersect.face.normal.clone();
                    let nx = bx + Math.round(normal.x);
                    let ny = by + Math.round(normal.y);
    ...
    stopMining() {
        this.isMining = false; this.miningTimer = 0; this.targetBlockPos = null;
        document.getElementById('mining-ui').style.display = 'none'; document.getElementById('mining-progress').style.width = '0%';
        this.miningIndicator.material.opacity = 0;
    }

    checkCollision(x, y, z) {
        const radius = 0.25; 
        const feetY = y - 1.5; const headY = y + 0.2;
    ...
    update(delta) {
        ...
        let targetX, targetZ;
        if (this.isMobile && (this.joyMove.x !== 0 || this.joyMove.y !== 0)) {
            targetX = this.joyMove.x * this.speed; targetZ = -this.joyMove.y * this.speed; 
        } else {
            this._input.set( Number(this.moveRight) - Number(this.moveLeft), 0, Number(this.moveForward) - Number(this.moveBackward) ).normalize();
            targetX = this._input.x * this.speed; targetZ = this._input.z * this.speed;
        }
        ...
        this.camera.position.x += (dxRight + dxForward);
        if (this.checkCollision(this.camera.position.x, this.camera.position.y, this.camera.position.z)) { this.camera.position.x -= (dxRight + dxForward); }

        this.camera.position.z += (dzRight + dzForward);
        if (this.checkCollision(this.camera.position.x, this.camera.position.y, this.camera.position.z)) { this.camera.position.z -= (dzRight + dzForward); }

        this.velocity.y -= this.gravity * delta;
        const yMove = this.velocity.y * delta;
        const ySteps = Math.max(1, Math.ceil(Math.abs(yMove) / 0.1)); 
        const yStepAmt = yMove / ySteps;
        ...
        for (let i = 0; i < ySteps; i++) {
            ...
            if (this.velocity.y < 0) { 
                if (this.checkCollision(this.camera.position.x, this.camera.position.y, this.camera.position.z)) {
                    let highestBlockY = Math.floor(this.camera.position.y - 1.5 + 0.5);
                    this.camera.position.y = highestBlockY + 0.5 + 1.5; 
                    this.velocity.y = 0; this.canJump = true; break; 
                } else { this.canJump = false; }
            } else if (this.velocity.y > 0) { 
                if (this.checkCollision(this.camera.position.x, this.camera.position.y, this.camera.position.z)) {
                    this.camera.position.y = Math.floor(this.camera.position.y + 0.2 + 0.5) - 0.5 - 0.2; 
        ...
        if (this.canJump && horizSpeed > 1) {
            ...
            if (this.footstepTimer > 0.35) { 
                let floorType = this.world.getBlockType(Math.floor(this.camera.position.x), Math.floor(this.camera.position.y - 1.6), Math.floor(this.camera.position.z));
                if(floorType === 'stone' || floorType === 'bedrock') { if(AudioSys && AudioSys.stepStone) AudioSys.stepStone(); } else { if(AudioSys && AudioSys.stepGrass) AudioSys.stepGrass(); } 
                this.footstepTimer = 0; 
            }
        }
        ...
        for (let i = this.world.drops.length - 1; i >= 0; i--) {
            ...
            if (drop.pickupDelay <= 0 && this.camera.position.distanceTo(drop.mesh.position) < 1.8) { 
                if (this.pickupItem(drop.type)) { this.world.dropGroup.remove(drop.mesh); drop.mesh.geometry.dispose(); this.world.drops.splice(i, 1); }
            }
        }
        ...
        if (this.shakeIntensity > 0) {
            ...
            this.shakeIntensity = Math.max(0, this.shakeIntensity - delta * 3);
        }
        ...
        if (this.canJump && horizSpeed > 1) { 
            ...
            let swing = Math.sin(this.legSwingTimer) * 0.5;
            targetLegLX = swing; targetLegRX = -swing; 
            targetArmLX = -swing; targetArmRX = swing; 
        }

        let pitch = this._euler.x;
        this.head.rotation.x = pitch;

        let targetFPSArmX = -Math.PI / 4;
        ...
        if (this.attackAnimTimer > 0) {
            ...
            let mine = Math.abs(Math.sin(performance.now() * 0.015)) * 0.5;
            targetFPSArmX = -Math.PI / 4 + mine;
            targetArmRX = pitch + mine;
            ...
            for (let intersect of intersects) {
                ...
                if (currentPos === this.targetBlockPos) {
                    ...
                    this.miningIndicator.position.set(bx, by, bz);
                    this.miningIndicator.material.opacity = (this.miningTimer / this.miningDurability) * 0.8;

                    let progressEl = document.getElementById('mining-progress');
                    if(progressEl) progressEl.style.width = `${Math.min(100, (this.miningTimer / this.miningDurability) * 100)}%`;
        ...
        this.arm.rotation.x = THREE.MathUtils.lerp(this.arm.rotation.x, targetFPSArmX, 15 * delta);
        this.armR_3rd.rotation.x = THREE.MathUtils.lerp(this.armR_3rd.rotation.x, targetArmRX, 15 * delta);
        this.armL.rotation.x = THREE.MathUtils.lerp(this.armL.rotation.x, targetArmLX, 15 * delta);
        this.legR.rotation.x = THREE.MathUtils.lerp(this.legR.rotation.x, targetLegRX, 15 * delta);
        this.legL.rotation.x = THREE.MathUtils.lerp(this.legL.rotation.x, targetLegLX, 15 * delta);
