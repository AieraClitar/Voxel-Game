...
export class Player {
    constructor(camera, domElement, world) {
        ...
        this.inLava = false; this.lavaDamageTimer = 0;
        ...
        document.addEventListener('keydown', (e) => {
            ...
            switch(e.code){ 
                ...
                case 'Space': 
                    ...
                    else if (this.inLava) { this.velocity.y = 2.0; } 
                    break; 
    ...
    onInteract(buttonIdx) {
        ...
        if (buttonIdx === 0 && this.attackAnimTimer <= 0) {
            if(tool === 'gun') { if(AudioSys && AudioSys.shootGun) AudioSys.shootGun(); this.shakeIntensity = 0.4; this.muzzleFlash.intensity = 15; this.attackAnimTimer = 0.3; }
            else if(tool === 'crossbow') { if(AudioSys && AudioSys.shootCrossbow) AudioSys.shootCrossbow(); this.shakeIntensity = 0.3; this.attackAnimTimer = 1.2; } 
            else if(tool === 'bow') { if(AudioSys && AudioSys.shootBow) AudioSys.shootBow(); this.shakeIntensity = 0.2; this.attackAnimTimer = 0.6; }
            else { if(AudioSys && AudioSys.playNoise) AudioSys.playNoise(0.1, 0.05, 500); this.attackAnimTimer = 0.25; }
        } else if (buttonIdx === 0 && this.attackAnimTimer > 0) return; 
        ...
        for (let intersect of intersects) {
            ...
            if (intersect.object.isInstancedMesh) { const pos = intersect.object.userData.positions[intersect.instanceId]; if (!pos) continue; bx = pos.x; by = pos.y; bz = pos.z; } 
                ...
                if (buttonIdx === 0) { 
                    ...
                    if (tool === 'gun') dmg = 50; if (tool === 'crossbow') dmg = 35; if (tool === 'bow') dmg = 20;       
            ...
            if (buttonIdx === 0) { 
                ...
                if (selected && selected.type !== null && selected.count > 0 && !['stick', 'bow', 'crossbow', 'gun', 'wooden_sword', 'stone_sword', 'wooden_pickaxe', 'stone_pickaxe', 'wooden_axe', 'stone_axe', 'wooden_shovel', 'stone_shovel'].includes(selected.type)) {
                    ...
                    // 🔧 FIX: Strict check to block placing Torches or ANY solid item in water/lava
    ...
    update(delta) {
        ...
        this.inLava = bodyType === 'lava' || feetType === 'lava'; 
        let onIce = feetType === 'ice' && !this.inWater && !this.inLava && this.canJump;
        ...
        if (this.inLava) {
            if (!this.lavaDamageTimer) this.lavaDamageTimer = 0;
            ...
            if (this.lavaDamageTimer > 0.5) { 
                ...
                const flash = document.createElement('div'); flash.style.position = 'absolute'; flash.style.top = '0'; flash.style.left = '0'; flash.style.width = '100%'; flash.style.height = '100%'; flash.style.backgroundColor = 'rgba(255, 69, 0, 0.4)'; flash.style.pointerEvents = 'none'; flash.style.zIndex = '9999'; flash.style.transition = 'opacity 0.5s';
                document.body.appendChild(flash); setTimeout(() => { flash.style.opacity = '0'; setTimeout(()=>flash.remove(), 500); }, 50);
            }
        }
